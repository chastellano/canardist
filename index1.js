...
const { dealMission, populateThings, shuffle } = require('./server/helpers.js');
const activeRooms = require('./server/activeRooms');
...
io.on('connection', socket => {
    ...
    if (!activeRooms.includes(gameId)) {
        activeRooms.push(gameId);
    }
    console.log('SERVER 69: ' + activeRooms);
    let handle;
    ...
    socket.on('nameCheck', name => {
        ...
        if (available) {
            ...
            if (rooms[gameId]['players'][name]) {
                ...
                console.log( `There are ${activeRooms.length} active rooms: ${activeRooms}`);
    ...
    socket.on('disconnect', () => {
        ...
        if (rooms[gameId] && rooms[gameId]['players']) {
            ...
            if (num === 0) {
                if (activeRooms.indexOf(gameId) !== -1) {
                    const i = activeRooms.indexOf(gameId);
